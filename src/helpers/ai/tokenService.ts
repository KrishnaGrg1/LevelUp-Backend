...
const TOKEN_COST_PER_CHAT = Number.isFinite(parsedCost) && parsedCost > 0 ? parsedCost : 1;
...
export type ConsumeTokensResult =
  ...
  | { ok: false; remainingTokens: number; cost: number; reason: 'insufficient' };
...
export async function consumeTokens(
  ...
  return client.$transaction(async (tx) => {
    ...
    if (updated.count === 0) {
      ...
      if (!user) {
        logger.warn('[AI Tokens] User not found while consuming tokens', { userId, cost });
        throw new Error('USER_NOT_FOUND');
      }

      return { ok: false, remainingTokens: user.tokens, cost, reason: 'insufficient' } as const;
    }

    const refreshed = await tx.user.findUniqueOrThrow({ where: { id: userId }, select: { tokens: true } });
 ...
 */
export async function refundTokens(userId: string, amount: number = TOKEN_COST_PER_CHAT): Promise<number> {
  ...
  logger.warn('[AI Tokens] Refunded tokens after failure', { userId, amount, remaining: updated.tokens });
