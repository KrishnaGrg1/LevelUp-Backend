...
const createClan = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Create the clan
    const clan = await client.clan.create({
      data: {
        name,
        description,
    ...
    // Add the owner to the clan members
    await client.communityMember.update({
      where: { id: member.id },
      data: {
        clanId: clan.id,
        role: 'ADMIN', // Owner is admin in clan
      },
    });
    ...
    return res.status(500).json(
      makeErrorResponse(
        new Error('Failed to create clan'),
        'error.clan.failed_to_create',
        lang,
        500
      )
    );
...
const getClanMembers = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(makeSuccessResponse(members, 'success.clan.members_retrieved', lang, 200));
  } catch (e: unknown) {
    const lang = (req.language as Language) || 'eng';
    return res.status(500).json(
      makeErrorResponse(
...
const updateClan = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await client.clan.update({
      where: { id: clanId },
      data: { name, description, isPrivate, limit },
    });
...
const getUserClans = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(makeSuccessResponse(clans, 'success.clan.user_clans_retrieved', lang, 200));
  } catch (e: unknown) {
    const lang = (req.language as Language) || 'eng';
    return res.status(500).json(
      makeErrorResponse(
