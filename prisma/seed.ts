...
const userNames = [
  'james_anderson', 'mary_johnson', 'robert_williams', 'patricia_brown', 'michael_jones',
  'linda_garcia', 'william_martinez', 'barbara_rodriguez', 'david_davis', 'elizabeth_miller',
  'richard_wilson', 'jennifer_moore', 'joseph_taylor', 'susan_thomas', 'thomas_jackson',
  'jessica_white', 'charles_harris', 'sarah_martin', 'christopher_thompson', 'karen_lee',
  'daniel_walker', 'nancy_hall', 'matthew_allen', 'lisa_young', 'anthony_king',
  'betty_wright', 'mark_lopez', 'sandra_hill', 'donald_scott', 'ashley_green',
  'steven_adams', 'kimberly_baker', 'paul_nelson', 'emily_carter', 'andrew_mitchell',
  'donna_perez', 'joshua_roberts', 'michelle_turner', 'kenneth_phillips', 'carol_campbell',
  'kevin_parker', 'amanda_evans', 'brian_edwards', 'melissa_collins', 'george_stewart',
  'deborah_morris', 'edward_rogers', 'stephanie_reed', 'ronald_cook', 'rebecca_morgan'
];
...
const communityNames = [
  'Web Developers Hub', 'Fitness Warriors', 'AI & Machine Learning', 'Gaming Legends', 'Creative Minds',
  'Data Science Masters', 'Mobile App Creators', 'Cloud Computing Pros', 'Cybersecurity Squad', 'DevOps Engineers',
  'Python Programmers', 'JavaScript Enthusiasts', 'Yoga & Meditation', 'Marathon Runners', 'Weightlifting Club',
  'Nutrition & Diet', 'Mental Health Support', 'Book Lovers Society', 'Photography Artists', 'Digital Marketing Gurus',
  'Startup Founders', 'Blockchain Builders', 'UI/UX Designers', 'Content Creators', 'Music Producers',
  'Video Editors', ' 3D Modeling Artists', 'Language Learners', 'Math & Science', 'Writing Workshop',
  'Chess Masters', 'E-Sports Champions', 'Board Game Enthusiasts', 'Cooking & Recipes', 'Travel Explorers',
  'Environmental Activists', 'Volunteer Network', 'Pet Lovers Unite', 'Gardening Community', 'DIY Crafters',
  'Film & Cinema Club', 'Stand-up Comedy', 'Podcast Creators', 'Investment & Finance', 'Real Estate Network',
  'Legal Professionals', 'Medical Community', 'Teachers Network', 'Architecture & Design', 'Automotive Enthusiasts'
];

const clanPrefixes = ['Elite', 'Alpha', 'Shadow', 'Phoenix', 'Dragon', 'Thunder', 'Storm', 'Mystic', 'Iron', 'Golden'];
const clanSuffixes = ['Warriors', 'Knights', 'Squad', 'Legion', 'Alliance', 'Guild', 'Force', 'Clan', 'Order', 'Brotherhood'];

const questTemplates = {
  daily: [
    'Complete a 30-minute coding session', 'Review and refactor old code', 'Learn a new programming concept',
    'Solve 3 coding challenges', 'Read technical documentation', 'Write unit tests for a module',
    'Optimize database queries', 'Implement a new feature', 'Debug and fix 2 issues', 'Code review for team',
    'Complete 30 minutes of cardio', 'Do 50 push-ups', 'Practice yoga for 20 minutes', 'Drink 8 glasses of water',
    'Meal prep healthy lunch', 'Take a 15-minute walk', 'Stretch for 10 minutes', 'Get 8 hours of sleep',
    'Read 20 pages of a book', 'Practice a new language for 15 minutes', 'Meditate for 10 minutes',
    'Journal your thoughts', 'Learn something new', 'Help a community member', 'Share knowledge with others'
  ],
  weekly: [
    'Build a complete REST API', 'Create a full-stack application', 'Learn a new framework',
    'Complete an online course module', 'Contribute to open source', 'Write a technical blog post',
    'Attend a tech meetup or webinar', 'Mentor a junior developer', 'Complete a major project milestone',
    'Run 10 kilometers', 'Attend 3 fitness classes', 'Meal prep for the entire week', 'Try a new workout routine',
    'Read a complete book', 'Complete a personal project', 'Network with 5 new people', 'Learn a new skill',
    'Volunteer for 4 hours', 'Create content for social media', 'Review and update your goals'
  ]
};

async function main() {
  console.log('Starting database seeding with 50 users, 50 communities, and extensive quests...');
  ...
  const heardAbout = ['Social Media', 'Friend Referral', 'Search Engine', 'Advertisement', 'Blog Post'];
  const goals = [
    'Improve coding skills', 'Get fit and healthy', 'Learn new technologies', 'Build a portfolio',
    'Network with professionals', 'Start a side project', 'Career advancement', 'Personal growth'
  ];
  ...
  for (let i = 1; i < users.length; i++) {
    ...
    await prisma.userOnboarding.create({
      data: {
        ...
        experience: experiences[Math.floor(Math.random() * experiences.length)] as any,
        categories: {
          connect: categoryIds,
        },
      },
  ...
  for (let i = 0; i < communities.length; i++) {
    ...
    if (ownerUser) {
      ...
      const member = await prisma.communityMember.create({
        data: {
          ...
          level: memberLevel,
          status: memberStatuses[Math.floor(Math.random() * memberStatuses.length)] as any,
    ...
    // Add random members
    while (memberIndices.size < numMembers && memberIndices.size < users.length) {
      const userIdx = Math.floor(Math.random() * users.length);
      if (!memberIndices.has(userIdx)) {
        memberIndices.add(userIdx);
        const memberLevel = Math.floor(users[userIdx].level * 0.6) + Math.floor(Math.random() * 5);
        const member = await prisma.communityMember.create({
          data: {
  ...
  for (let i = 0; i < communities.length; i++) {
    const communityUserMembers = communityMembers.filter(cm => cm.communityId === communities[i].id);
    
    for (let j = 0; j < 10; j++) {
      const ownerMember = communityUserMembers[Math.floor(Math.random() * communityUserMembers.length)];
      ...
      const suffix = clanSuffixes[Math.floor(Math.random() * clanSuffixes.length)];
      const clanName = `${communities[i].name} - ${prefix} ${suffix}`;
      const slug = clanName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
      
      clans.push(await prisma.clan.create({
        data: {
          name: clanName,
          slug: `${slug}-${i}-${j}`,
          ...
          stats: { memberCount: 0, battlesWon: Math.floor(Math.random() * 20) },
        },
      }));
    }
  }
  ...
  for (let i = 0; i < clans.length; i++) {
    const clan = clans[i];
    const communityUserMembers = communityMembers.filter(cm => cm.communityId === clan.communityId);
    ...
    clanMembers.push(await prisma.clanMember.create({
      data: {
        userId: clan.ownerId,
        clanId: clan.id,
    ...
    while (clanMemberIndices.size < numClanMembers && clanMemberIndices.size < communityUserMembers.length) {
      const member = communityUserMembers[Math.floor(Math.random() * communityUserMembers.length)];
  ...
  for (let i = 0; i < users.length; i++) {
    const user = users[i];
    const userCommunityMembers = communityMembers.filter(cm => cm.userId === user.id);
    ...
    for (let j = 0; j < 3; j++) {
      const communityMember = userCommunityMembers[Math.floor(Math.random() * userCommunityMembers.length)];
      const questDesc = questTemplates.daily[Math.floor(Math.random() * questTemplates.daily.length)];
      ...
      const startTime = new Date(now.getTime() - (estimatedMins + 10) * 60 * 1000);
      ...
      await prisma.quest.create({
        data: {
          ...
          viewedAt: now,
          startedAt: startTime,
          completedAt: isCompleted ? new Date(now.getTime() + Math.floor(Math.random() * 3 * 60 * 60 * 1000)) : null,
    ...
    for (let j = 0; j < 2; j++) {
      ...
      await prisma.quest.create({
        data: {
          ...
          periodSeq: j + 1,
          viewedAt: yesterday,
          startedAt: isCompleted ? yesterday : (Math.random() < 0.5 ? yesterday : null),
          completedAt: isCompleted ? new Date(yesterday.getTime() + Math.floor(Math.random() * 5 * 60 * 60 * 1000)) : null,
    ...
    const communityMember1 = userCommunityMembers[Math.floor(Math.random() * userCommunityMembers.length)];
    const questDesc1 = questTemplates.daily[Math.floor(Math.random() * questTemplates.daily.length)];
    ...
    await prisma.quest.create({
      data: {
        ...
        completedAt: isCompleted1 ? new Date(dayBeforeYesterday.getTime() + Math.floor(Math.random() * 5 * 60 * 60 * 1000)) : null,
        estimatedMinutes: 30 + Math.floor(Math.random() * 60),
      },
    });
    questCount++;
    ...
    for (let j = 0; j < 2; j++) {
      const communityMember = userCommunityMembers[Math.floor(Math.random() * userCommunityMembers.length)];
      const questDesc = questTemplates.weekly[Math.floor(Math.random() * questTemplates.weekly.length)];
      ...
      await prisma.quest.create({
        data: {
          ...
          completedAt: isCompleted ? new Date(now.getTime() + Math.floor(Math.random() * 10 * 60 * 60 * 1000)) : null,
    ...
    for (let j = 0; j < 2; j++) {
      ...
      await prisma.quest.create({
        data: {
          ...
          periodSeq: j + 1,
          viewedAt: lastWeek,
          startedAt: isCompleted ? lastWeek : (Math.random() < 0.5 ? lastWeek : null),
          completedAt: isCompleted ? new Date(lastWeek.getTime() + Math.floor(Math.random() * 15 * 60 * 60 * 1000)) : null,
          estimatedMinutes: 120 + Math.floor(Math.random() * 180),
        },
      });
      questCount++;
    ...
    const communityMember2 = userCommunityMembers[Math.floor(Math.random() * userCommunityMembers.length)];
    const questDesc2 = questTemplates.weekly[Math.floor(Math.random() * questTemplates.weekly.length)];
    ...
    await prisma.quest.create({
      data: {
        ...
        completedAt: isCompleted2 ? new Date(twoWeeksAgo.getTime() + Math.floor(Math.random() * 20 * 60 * 60 * 1000)) : null,
        estimatedMinutes: 120 + Math.floor(Math.random() * 180),
      },
    });
    questCount++;
  ...
  const subjects = ['BUG_REPORT', 'FEATURE_REQUEST', 'ACCOUNT_ISSUE', 'COMMUNITY_MANAGEMENT', 'QUEST_MANAGEMENT', 'OTHER'];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    await prisma.ticket.create({
      data: {
        ...
        expectedDateOfCompletion: Math.random() < 0.5 ? new Date(now.getTime() + Math.floor(Math.random() * 7 * 24 * 60 * 60 * 1000)) : null,
      },
    });
  }
  ...
  for (let i = 0; i < Math.min(30, communities.length); i++) {
    ...
    const members = communityMembers.filter(cm => cm.communityId === community.id);
  ...
  console.log('Users: james_anderson@levelup.com, mary_johnson@levelup.com, etc.');
